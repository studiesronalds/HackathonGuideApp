  ...
  public async up(queryRunner: QueryRunner): Promise<any> {
    await queryRunner.query(`
      CREATE OR REPLACE FUNCTION make_tsvector(
          name_blr TEXT, name_ru TEXT, name_en TEXT, 
          description_blr TEXT, description_ru TEXT, description_en TEXT
        )
        RETURNS tsvector AS $$
      BEGIN
        RETURN (
          setweight(to_tsvector('russian', name_blr),'A') ||
          setweight(to_tsvector('russian', name_ru),'A') ||
          setweight(to_tsvector('russian', name_en),'A') ||
          setweight(to_tsvector('russian', description_blr),'B') ||
    ...
    await queryRunner.query(`
      ...
      USING gin(make_tsvector(name_blr, name_ru, name_en, description_blr, description_ru, description_en));
    `);
    await queryRunner.query(`
